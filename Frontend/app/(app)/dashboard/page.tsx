    ...
    <div className="space-y-4 animate-pulse" aria-hidden="true">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className="h-48 rounded-2xl bg-slate-200 dark:bg-slate-700" />
        ))}
      </div>
    </div>
  );
      ...
      <main className="mx-auto max-w-6xl p-4 md:p-8">
        <header className="mb-6 flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            <Link href="/monthly" className="rounded-xl bg-bankBlue px-4 py-2 text-sm font-medium text-white hover:bg-bankBlue/90 transition">
              Monthly View
            </Link>
            <Link href="/analytics" className="rounded-xl bg-bankBlue px-4 py-2 text-sm font-medium text-white hover:bg-bankBlue/90 transition">
              Analytics
            </Link>
            <ThemeToggle />
            <button
              type="button"
              onClick={() => signOutUser()}
              className="rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 px-4 py-2 text-sm text-slateInk dark:text-slate-100 hover:bg-slate-50 dark:hover:bg-slate-600 transition"
            >
              Sign Out
            </button>
          </div>
        ...
        {error ? (
          ...
          <div className="flex flex-col items-center justify-center rounded-2xl bg-white dark:bg-slate-800 p-12 text-center shadow-panel">
            <p className="text-slate-600 dark:text-slate-400">No financial data found. Please try again later.</p>
          </div>
        ) : (
